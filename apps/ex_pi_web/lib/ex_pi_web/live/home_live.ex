defmodule ExPiWeb.HomeLive do
  ...
  def render(assigns) do
    ...
    <div class="max-w-6xl mx-auto py-12 px-6 text-on-surface">
      ...
      <div
        :if={Enum.empty?(@repos)}
        class="text-center py-24 bg-surface-container-low rounded-3xl border-2 border-dashed border-outline-variant"
      >
      ...
      <div :if={!Enum.empty?(@repos)} class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <.dm_card
          :for={repo <- @repos}
          variant="bordered"
          class="group interactive hover:shadow-xl transition-all duration-300"
        >
          <:title>
          ...
          <:action>
            <div class="flex gap-2 w-full">
              <.dm_link
                ...
                class="dm-btn dm-btn--outline flex-1 text-center"
              >
                Open
              </.dm_link>
              <.dm_btn
                id={"remove-repo-#{Base.encode16(:crypto.hash(:md5, repo["path"]), case: :lower)}"}
      ...
      <.dm_modal :if={@show_add_modal} id="add-project-modal">
        ...
        <:body>
          <div class="flex flex-col h-[500px]">
            ...
            <div class="flex-1 overflow-y-auto border border-outline-variant rounded-xl divide-y divide-outline-variant bg-surface-container-low">
              ...
              >
                <.dm_mdi
                  name={if entry.is_dir, do: "folder", else: "file-outline"}
                  class={if entry.is_dir, do: "text-secondary", else: "text-on-surface-variant opacity-40"}
                />
                <span class={"text-sm truncate flex-1 #{if !entry.is_dir, do: "opacity-60"}"}>
                  {entry.name}
                </span>
                <.dm_mdi
                  :if={entry.is_dir}
                  name="chevron-right"
                  class="w-4 h-4 opacity-0 group-hover:opacity-40 transition-opacity"
                />
              </div>
            </div>

            <div class="mt-6 pt-4 border-t border-outline-variant">
              <p class="text-[10px] opacity-40 text-on-surface mb-2 uppercase tracking-widest font-bold">
                Selected Path
              </p>
        ...
        <:footer>
          <.dm_btn id="cancel-add-btn" phx-click="hide_add_modal" phx-hook="WebComponentHook" variant="ghost">
            Cancel
          </.dm_btn>
          <.dm_btn
            id="confirm-add-btn"
            phx-click="browser_confirm"
            phx-hook="WebComponentHook"
            variant="primary"
          >
            Add This Directory
